...
static HAL_StatusTypeDef LCD_Command(LCD_HandleTypeDef *hlcd, uint8_t cmd)
{
	...
	return HAL_I2C_Master_Transmit(hlcd->hi2c, LCD_ADDRESS << 1, data, 2, HAL_MAX_DELAY);
}

static HAL_StatusTypeDef LCD_Write(LCD_HandleTypeDef* hlcd, uint8_t data) {
    uint8_t buf[2] = {0x40, data};
    return HAL_I2C_Master_Transmit(hlcd->hi2c, LCD_ADDRESS << 1, buf, 2, HAL_MAX_DELAY);
}

static HAL_StatusTypeDef LCD_SetReg(LCD_HandleTypeDef* hlcd, uint8_t reg, uint8_t data) {
    uint8_t buf[2] = {reg, data};
    return HAL_I2C_Master_Transmit(hlcd->hi2c, hlcd->rgb_chip_addr << 1, buf, 2, HAL_MAX_DELAY);
}
...
HAL_StatusTypeDef LCD_Init(LCD_HandleTypeDef* hlcd, I2C_HandleTypeDef* hi2c) {
    ...
    uint8_t rgbData = 0;
    status = HAL_I2C_Master_Transmit(hlcd->hi2c, RGB_ADDRESS_V5 << 1, &rgbData, 1, HAL_MAX_DELAY);
    if(status == HAL_OK) {
    	...
    	status = HAL_I2C_Master_Transmit(hlcd->hi2c, RGB_ADDRESS << 1, &rgbData, 1, HAL_MAX_DELAY);
    	if(status == HAL_OK) {
