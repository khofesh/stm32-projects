...
/* External byte pool for memory allocation */
...
static int32_t icmp_ping(struct nx_bsd_sockaddr *pS_Addr, uint32_t S_Addr_len, uint32_t Count, uint32_t Timeout,
		int32_t Response[]);
static int32_t ping_gethostbyname(struct nx_bsd_sockaddr *pS_Addr, const char *Name);
		...
		int32_t Response[])
{
	...
	struct nx_bsd_timeval timeout_opt = {0};
	...
	const INT sock = nx_bsd_socket(AF_INET, SOCK_RAW, IPPROTO_ICMP);
	MSG_DEBUG("-> %" PRId32 "\n", (int32_t)sock);
	...
	else if (nx_bsd_setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &timeout_opt, sizeof(timeout_opt)) < 0)
	{
	...
	if (ret == 0)
	{
		for (uint32_t i = 0; i < Count; i++)
		{
			...
			if (nx_bsd_sendto(sock, (CHAR *)p_echo_buffer, (INT) ping_size, 0, pS_Addr, (INT) S_Addr_len) < 0)
			{
			...
			do
			{
				struct nx_bsd_sockaddr_in s_addr_in_from = {0};
				INT s_addr_in_from_len = sizeof(s_addr_in_from);
				CHAR buf[64] = "";
				const INT len = nx_bsd_recvfrom(sock, buf, (INT) ping_size, 0, (struct nx_bsd_sockaddr *)&s_addr_in_from, &s_addr_in_from_len);
				const uint32_t now = HAL_GetTick();

				MSG_DEBUG("Received from %s\n", nx_bsd_inet_ntoa(s_addr_in_from.sin_addr));
...
{
	int32_t ret = -1;
	struct nx_bsd_addrinfo hints = {0};
	struct nx_bsd_addrinfo *host_info = NULL;

	hints.ai_family = pS_Addr->sa_family;
	hints.ai_socktype = SOCK_DGRAM;
	hints.ai_flags = AI_PASSIVE;

	if (0 == nx_bsd_getaddrinfo((const CHAR *)Name, NULL, &hints, &host_info))
	{
		if (host_info->ai_family == pS_Addr->sa_family)
		{
			struct nx_bsd_sockaddr_in *const s_addr_in = (struct nx_bsd_sockaddr_in *) pS_Addr;

			s_addr_in->sin_addr.s_addr = ((struct nx_bsd_sockaddr_in *)host_info->ai_addr)->sin_addr.s_addr;

			ret = 0;
		}
		nx_bsd_freeaddrinfo(host_info);
	}
	return ret;
}
...
int32_t ping_cmd(int32_t argc, char *argv[])
{
	...
	struct nx_bsd_sockaddr_in s_addr_in = {0};
	...
	if (ping_gethostbyname((struct nx_bsd_sockaddr *)&s_addr_in, server_name) < 0)
	{
	...
	else
	{
		MSG_INFO("Pinging hostname \"%s\" with %s:%" PRIu32 "\n",
				server_name, nx_bsd_inet_ntoa(s_addr_in.sin_addr), (uint32_t)s_addr_in.sin_port);
	}

	if (icmp_ping((struct nx_bsd_sockaddr *)&s_addr_in, sizeof(s_addr_in), PING_ITERATION, PING_DELAY_S, ping_res) >= 0)
	{
