...
static mqtt_mode_t current_mqtt_mode = MQTT_MODE_PLAIN;
...
void bme280_mqtt_task(void)
{
	...
	// Initialize or reconnect MQTT if needed
	if (!mqtt_connected && (current_time - last_reconnect_attempt) > 10000) // Try reconnect every 10 seconds
	{
		last_reconnect_attempt = current_time;

		USER_LOG("Attempting MQTT connection...");

		// Configure MQTT based on current mode
		if (current_mqtt_mode == MQTT_MODE_TLS)
		{
			// Skip certificate verification for TLS
			mqtt_result = ESP_DMA_SendCommand("AT+CIPSSLCCONF=0,0\r\n", "OK", 2000);
			if (mqtt_result != ESP8266_OK)
			{
				USER_LOG("SSL configuration failed");
				return;
			}
			mqtt_result = ESP_MQTT_Init(MQTT_BROKER_HOST, MQTT_BROKER_PORT_TLS, MQTT_CLIENT_ID);
		}
		else
		{
			mqtt_result = ESP_MQTT_Init(MQTT_BROKER_HOST, MQTT_BROKER_PORT_PLAIN, MQTT_CLIENT_ID);
		}
		...
		mqtt_result = ESP_MQTT_SetAuth(MQTT_USERNAME, MQTT_PASSWORD);
		if (mqtt_result != ESP8266_OK)
		{
		...
		mqtt_result = ESP_MQTT_Connect();
		if (mqtt_result != ESP8266_OK)
		{
			USER_LOG("MQTT connection failed, will retry");
			return;
		}
	...
	// Skip publishing if MQTT is not connected
	if (!mqtt_connected)
	{
		return;
	}
	...
	if (mqtt_result != ESP8266_OK)
	{
		...
		// Only mark MQTT as disconnected, WiFi remains connected
		mqtt_connected = false;
		return;
	}
